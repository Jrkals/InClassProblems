...
struct Recipe{
    ...
    init(dictionary: [String: Any]){
        ...
        let originalURL = URL(string: originalURLString ?? "https://apple.com")
        let backupURL = URL(string: "https://www.apple.com")!
        let ingredients = ingredientArray.map { Ingredient(dictionary: $0) }
        
        self.init(name:name, ingredients: ingredients, steps: steps, timers: timers, imageURL: imageURL,
                  originalURL: originalURL ?? backupURL)
    }
